...
export default function EditDebtPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [formData, setFormData] = useState({
        ...
        installments: [] as Array<{ amount: string; date: string; isPaid?: boolean }>
    });
    ...
    useEffect(() => {
        const fetchDebt = async () => {
            ...
            try {
                const docRef = doc(db, "debts", resolvedParams.id);
                ...
                if (docSnap.exists()) {
                    ...
                    setFormData({
                        ...
                        firstInstallmentDate: formatDate(data.firstInstallmentDate),
                ...
                setError("Borç yüklenirken hata oluştu.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.isInstallment && (formData.type === "loan" || formData.type === "credit_card" || formData.type === "person")) {
                ...
                debtData.isInstallment = false;
                debtData.installmentCount = null;
                debtData.firstInstallmentDate = null;
                debtData.installments = null;
            }
    ...
    const handleInstallmentCountChange = (count: string) => {
        ...
        // Preserve existing amounts if possible
        ...
        const newInstallments = dates.map((date, index) => ({
            ...
            date,
            isPaid: formData.installments[index]?.isPaid || false
        }));
    ...
    const handleInstallmentAmountChange = (index: number, amount: string) => {
        ...
        // BUT only if we are creating new logic or if user explicitly wants to sync?
        // In edit mode, maybe we should be less aggressive with auto-sync to avoid overwriting custom changes?
        // I'll keep the logic but maybe it's fine.
        if (index === 0) {
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
                <Link href="/debts" className="p-2 hover:bg-white/10 rounded-full transition-colors">
                    <FaArrowLeft />
                </Link>
                <h1 className="text-3xl font-bold">Borç Düzenle</h1>
            </div>
